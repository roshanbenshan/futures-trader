...
async def scan_candidates(bal, recovery_mode):
    ...
    # ── 方向平衡器：最近3单同向则强制推反向 ──
    dir_history = state.setdefault('trade_directions', [])
    if len(dir_history) >= 3 and len(set(dir_history[-3:])) == 1:
        only_dir = dir_history[-1]
        opposite = 'LONG' if only_dir == 'SHORT' else 'SHORT'
        print(f"  🔄 方向平衡: 最近3单全是{only_dir}, 强制推{opposite}")
        if opposite == 'LONG':
            long_ok = True
            short_ok = False
        else:
...
async def execute_trade(best_trade, bal):
    ...
    # 记录方向到历史（供方向平衡器使用）
    state.setdefault('trade_directions', []).append(side)
    if len(state['trade_directions']) > 20:
        state['trade_directions'] = state['trade_directions'][-20:]
...
async def monitor_and_exit(sym, p, bal):
    ...
    # ── 开仓保护期：前300秒跳过反转预警/横扫/连续K线检测 ──
    pos_open_time = state['positions'].get(sym.lower(), {}).get('open_time', 0)
    time_in_pos = time.time() - pos_open_time if pos_open_time > 0 else 9999
    IS_YOUNG = time_in_pos < 300
    if IS_YOUNG:
        print(f"  🛡️ 保护期({int(time_in_pos)}s/300s): 仅SL/TP+主动风控有效")
    ...
    if klines_ok:
        ...
        if max_warn >= WARN_EXIT and not IS_YOUNG:
            pnl_str = f'${upnl:+.2f}'
            print(f"  🚨 反转预警! 平仓!")
            ...
            await clean_orphan_algos(sym)
            return True
        elif max_warn >= WARN_EXIT:
            print(f"  ⏸️ 保护期内: 反转预警忽略 (RSI{max_warn})")
    ...
    # ══════════════════════════════════════════════
    #   LAYER 6: 15m横扫+连续反向K线 (保护期内跳过)
    # ══════════════════════════════════════════════
    if klines_ok and is_short and not IS_YOUNG:
        klist_local = list(state['kline_data'][sym.lower()])
        # 只取已收盘K线（防当前形成中的蜡烛闪烁）
        closed = [k for k in klist_local if k.get('final', True)]
        if len(closed) >= 4:
            # ── SWEEP检测：只看已收盘的最后一根 ──
            last_closed = closed[-1]
            # 分母用K线开盘价而非entry（分母bug修复）
            chg = (last_closed['c'] - last_closed['o']) / last_closed['o'] * 100
            body = abs(last_closed['c'] - last_closed['o'])
            avg_body = sum(abs(k['c']-k['o']) for k in closed[-5:]) / 5
            if chg > SWEEP_CHG_MIN and body > avg_body * SWEEP_BODY_MULT:
                state.setdefault('exiting', set()).add(sym)
                print(f"  ⚡15m横扫(已收盘)! 平仓!")
            ...
            # ── CONS_BULL检测：只计开仓后新收盘的阳线 ──
            entry_kline_ts = state['positions'].get(sym.lower(), {}).get('entry_kline_ts', 0)
            new_bulls = sum(1 for k in closed if k['t'] > entry_kline_ts and k['c'] > k['o'])
            if new_bulls >= CONSISTENT_BULL:
                state.setdefault('exiting', set()).add(sym)
                print(f"  🔴 开仓后{new_bulls}根新阳线! 平仓!")
...
async def run_trade_logic():
    ...
    while state['running']:
        try:
            ...
            if r:
                sf = r['sym'].lower()
                # 记录开仓时最新已收盘K线时间戳（用于CONS_BULL按开仓后计）
                entry_kts = 0
                ekd = state['kline_data'].get(sf, [])
                if ekd:
                    closed_kl = [k for k in ekd if k.get('final', True)]
                    if closed_kl:
                        entry_kts = closed_kl[-1]['t']
                state['positions'][sf] = {
                    'positionAmt': str(r['qty'] if r['side'] == 'LONG' else -r['qty']),
                    'entryPrice': str(r['entry']), 'unRealizedProfit': '0',
                    'open_time': time.time(),
                    'entry_kline_ts': entry_kts
                }
        except Exception as e:
            import traceback
            print(f"  ⚠️ 循环异常: {e}")
